...
struct MutezContractBox {
    ...
    /**
     Call MutezContract with specified params.
     **Important:**
     Params are in the order of how they are specified in the Tezos structure tree
    */
    func call(_ param1: Mutez) -> ContractMethodInvocation {
    ...
    @discardableResult
    func status(completion: @escaping RPCCompletion<MutezContractStatus>) -> Cancelable? {
        let endpoint = "/chains/main/blocks/head/context/contracts/" + at
        return tezosClient.sendRPC(endpoint: endpoint, method: .get, completion: completion)
    }
}

/// Status data of MutezContract
struct MutezContractStatus: Decodable {
    /// Balance of MutezContract in Tezos
    let balance: Tez
    /// Is contract spendable
    let spendable: Bool
    /// MutezContract's manager address
    let manager: String
    /// MutezContract's delegate
...
extension TezosClient {
    /**
     This function returns type that you can then use to call MutezContract specified by address.
