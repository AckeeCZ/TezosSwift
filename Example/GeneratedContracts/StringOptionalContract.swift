...
struct StringOptionalContractBox {
    ...
    /**
     Call StringOptionalContract with specified params.
     **Important:**
    ...
    */
    ...
    @discardableResult
    func status(completion: @escaping RPCCompletion<StringOptionalContractStatus>) -> Cancelable? {
        let endpoint = "/chains/main/blocks/head/context/contracts/" + at
        return tezosClient.sendRPC(endpoint: endpoint, method: .get, completion: completion)
    }
}

/// Status data of StringOptionalContract
struct StringOptionalContractStatus: Decodable {
    /// Balance of StringOptionalContract in Tezos
    let balance: Tez
    /// Is contract spendable
    let spendable: Bool
    /// StringOptionalContract's manager address
    let manager: String
    /// StringOptionalContract's delegate
...
extension TezosClient {
    /**
     This function returns type that you can then use to call StringOptionalContract specified by address.

     - Parameter at: String description of desired address.

     - Returns: Callable type to send Tezos with.
    */
    func stringOptionalContract(at: String) -> StringOptionalContractBox {
        return StringOptionalContractBox(tezosClient: self, at: at)
    }
}