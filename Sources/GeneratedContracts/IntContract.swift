...
struct IntContractBox {
	...
	func status(completion: @escaping RPCCompletion<IntContractStatus>) {
        let endpoint = "/chains/main/blocks/head/context/contracts/" + at
        tezosClient.sendRPC(endpoint: endpoint, method: .get, completion: completion)
    }
}

struct IntContractStatus: Decodable {
    let balance: Tez
    let spendable: Bool
    let manager: String
...
extension TezosClient {
    func intContract(at: String) -> IntContractBox {
        return IntContractBox(tezosClient: self, at: at)
    }
}
